...
function NewsletterForm() {
  ...
  return (
    <div className="w-full max-w-sm">
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="relative group">
          <input
            ...
            className="w-full bg-slate-800/50 text-slate-200 rounded-xl px-4 py-3 text-sm border border-slate-700/50 focus:border-purple-500/50 focus:outline-none focus:ring-2 focus:ring-purple-500/20 transition-all duration-300 placeholder-slate-400"
            aria-label="Email for newsletter"
            disabled={status === "loading"}
          />
          <div className="absolute inset-0 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-xl opacity-0 group-focus-within:opacity-100 transition-opacity duration-300 pointer-events-none" />
        </div>

        <button
          type="submit"
          disabled={status === "loading" || status === "success"}
          className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white px-4 py-3 rounded-xl text-sm font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-purple-500/50 transform hover:scale-[1.02] hover:-translate-y-0.5 shadow-lg hover:shadow-xl hover:shadow-purple-500/25"
        >
      ...
      {message && (
        <div
          className={`mt-3 p-3 rounded-lg text-sm transition-all duration-300 ${
            status === "success"
...
function SocialIcon({ href, icon: Icon, label, delay = 0, color = "purple" }) {
  ...
  return (
    <a
      href={href}
      className={`group relative p-3 bg-slate-800/30 border border-slate-700/50 rounded-xl text-slate-400 transition-all duration-300 transform hover:scale-110 hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-purple-500/50 ${colorClasses[color]}`}
      aria-label={label}
...
export default function Footer() {
  ...
  return (
    <>
      <footer className="relative w-full bg-slate-950 text-slate-300 mt-20">
        {/* Animated top border */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            <div className="lg:col-span-2 flex flex-col items-center md:items-start">
              <div className="flex items-center gap-3 mb-6 group">
                ...
                <span className="bg-gradient-to-r from-purple-400 via-pink-400 to-amber-400 bg-clip-text text-transparent font-bold text-3xl tracking-tight group-hover:scale-105 transition-transform duration-300">
                  MentorConnect
                </span>
              </div>

              <p className="text-slate-400 max-w-md text-center md:text-left leading-relaxed mb-6">
                ...
                {quickLinks.map((link, index) =>
                  availableRoutes.includes(link.href) ? (
                    <Link
                      key={link.href}
                      href={link.href}
                      className="block text-slate-400 hover:text-white transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-purple-500/50 rounded px-2 py-1 hover:bg-slate-800/30 transform hover:translate-x-1"
            ...
            <div className="flex flex-col items-center md:items-start">
              ...
              <div className="w-full">
                ...
                <p className="text-slate-400 text-sm mb-4">Get the latest updates and mentorship tips.</p>
                <NewsletterForm />
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="text-center md:text-left">
                ...
                <p className="text-slate-600 text-xs mt-1 flex items-center justify-center md:justify-start gap-1">
                  Built with <Heart size={12} className="text-red-400 animate-pulse" /> for the future of mentorship
                </p>
              </div>
